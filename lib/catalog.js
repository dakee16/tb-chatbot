...
import { getBrand, SHARED_FEED_FILE } from '../brands.js';
import { WEIGHT_SENTINEL } from './shipping.js';

const FEED_TTL_MS = 1000 * 60 * 60; // re-read the feed hourly
// All 9 brands share one catalog file, so cache by resolved file path (not by
// brand slug) — read and parse it once, reuse the same array for every brand.
const _fileCache = {}; // absPath -> { products, ts }

// Where the shared feed file lives. Override with FEED_DIR if needed.
const FEED_DIR = process.env.FEED_DIR || path.join(process.cwd(), 'data');

function localFeedPath(brand) {
  // A brand can still point at its own file later (e.g. once the owner
  // splits the feed) by setting brand.feedFile — otherwise everyone reads
  // the shared catalog.
  const file = brand.feedFile || SHARED_FEED_FILE;
