...
const _cache = {}; // slug -> { products, ts }

// Where local feed files live. Drop <slug>-feed.xml here (e.g. tilesbay-feed.xml)
// and it's used instead of fetching over HTTP. Override with FEED_DIR if needed.
const FEED_DIR = process.env.FEED_DIR || path.join(process.cwd(), 'data');

function localFeedPath(brand) {
  if (brand.feedFile) {
    return path.isAbsolute(brand.feedFile) ? brand.feedFile : path.join(process.cwd(), brand.feedFile);
  }
  return path.join(FEED_DIR, brand.slug + '-feed.xml');
}
...
function parseProduct(body) {
  ...
  return {
    ...
    unitPrice: (special != null && special > 0) ? special : price, // per 1 base unit
...
async function getCatalog(slug) {
  const now = Date.now();
  if (_cache[slug] && now - _cache[slug].ts < FEED_TTL_MS) return _cache[slug].products;
  ...
  try {
    if (fs.existsSync(localPath)) {
      ...
      _cache[slug] = { products, ts: now };
      console.log('[catalog] ' + slug + ': ' + products.length + ' products from ' + localPath);
      return products;
    }
  } catch (e) {
    console.error('[catalog] ' + slug + ' local read error: ' + e.message);
  }

  // 2) fall back to fetching the feed url (if the network path ever works).
  try {
    ...
    console.error('[catalog] ' + slug + ' feed error: ' + e.message);
    if (_cache[slug]) return _cache[slug].products; // serve stale on error
    return [];
  }
}
...
function haystack(p) {
  return (p.name + ' ' + p.material + ' ' + p.color + ' ' + p.size + ' ' +
    p.finish + ' ' + p.category).toLowerCase();
}
 ...
 */
export async function searchFeed(slug, query, limit = 6) {
